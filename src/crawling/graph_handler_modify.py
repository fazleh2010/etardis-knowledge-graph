...
class GraphHandler:
    ...
    def insert_connections(self, connections: Set) -> None:
        ...
        for idx, con in enumerate(list(connections)):
            ...
            subj_node = Node(uri=subj)
            subj_properties = OrderedDict()
            subj_properties["name"] =str(subj).replace("http://dbpedia.org/resource/","").replace("_"," ")
            subj_lbls = []
            subj_lbls =self.dbp_handler.get_original_class(subj)
            subj_properties["subclasses"] = subj_lbls
            subj_category = self.dbp_handler.get_mapped_class(subj)
            subj_node.add_label(subj_category)
            subj_node.update(subj_properties)

            obj_node = Node(uri=obj)
            obj_properties = OrderedDict()
            #label = ""
            #label = self.get_rdf_label(obj)
            obj_properties["name"] =str(obj).replace("http://dbpedia.org/resource/","").replace("_"," ")
            lbls_obj = []
            lbls_obj = self.dbp_handler.get_original_class(obj)
            obj_properties["subclasses"] = lbls_obj
            obj_label = self.dbp_handler.get_mapped_class(obj)
            obj_node.add_label(obj_label)
            obj_node.update(obj_properties)

            #obj_label = self.dbp_handler.get_mapped_class(obj)
            relation_name =f"has{obj_label.capitalize()}"
            index=index+1
            #print("idx::" + str(idx) + "str(node_subj)::" + str(subj_node) +" relation_name::"+str(relation_name)+" str(node_obj)::"+ str(obj_node) )
            #if relation_name.startswith("hasMiscellaneous"):
            #    continue
            if not self.node_matcher.match(uri=subj).exists():
               transaction.create(subj_node)
               self.graph.commit(tx=transaction)
            if not self.node_matcher.match(uri=obj).exists():
               transaction.create(obj_node)
               self.graph.commit(tx=transaction)
            if not self.graph.match(nodes=[obj_node, subj_node], r_type=relation_name).exists():
               nm_relationship = Relationship(subj_node, relation_name, obj_node)
