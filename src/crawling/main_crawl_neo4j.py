...
def main(uri_set=None):
    """
    Initialize crawling and insertion process foir a given dbpedia entry uri.
    :return:
    """
    do_cleanup = False
    update_index = True
    depth = 1
    search_depth = 1
    data_path = "/home/elahi/A-etardis/dataT/"
    #data_path ="/media/elahi/Elements1/A-Projects/etradis/resources/dataT/"
    done_uris_path=data_path+"updated_done_uris.csv"
    done_uris=get_categories(done_uris_path)

    d = DBPediaHandler()
    etradis_category = ["http://localhost:9999/etradis/CulturalArtifact"]
